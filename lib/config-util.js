...
Config.loadConfig = function (argv, cwd) {
  const rcPath = findUp.sync([argv.nycrcPath || '.nycrc', '.nycrc.json'], {
    cwd: guessCWD(cwd)
  })
  let config = {}
...
Config.buildYargs = function (cwd) {
  ...
  return Yargs([])
    ...
    .option('exclude', {
      ...
      describe:
        'a list of specific files and directories that should be excluded from coverage, glob patterns are supported, node_modules is always excluded',
      global: false
    })
    ...
    .example(
      '$0 --require @babel/register npm test',
      'instrument your tests with coverage and transpile with Babel'
    )
    .example(
      '$0 report --reporter=text-lcov',
