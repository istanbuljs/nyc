...
SourceMaps.prototype.extractAndRegister = function (code, metadata, hash) {
  var sourceMap = convertSourceMap.fromSource(code) || convertSourceMap.fromMapFileSource(code, path.dirname(metadata.relFile))
  if (sourceMap) {
    if (this.cache && hash) {
      var mapPath = path.join(this.cacheDirectory, hash + '.map')
      fs.writeFileSync(mapPath, sourceMap.toJSON())
    } else {
      this.sourceMapCache.registerMap(metadata.filename, sourceMap.sourcemap)
    }
  }
  return sourceMap
}
