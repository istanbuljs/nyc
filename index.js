...
class NYC {
  constructor(config) {
    ...
    // "require" is normalized by @istanbuljs/load-nyc-config,
    // unclear `[].concat` is still needed.
  ...
  _createTransform(ext) {
    ...
    if (this._eagerInstantiation) {
      opts.transform = this._transformFactory();
    } else {
      opts.factory = () => this._transformFactory();
    }
  ...
  _addHook(type) {
    ...
    // We do all match processing in transformer
    const dummyMatcher = () => true;
  ...
  _wrapExit() {
    ...
    // We always want to write coverage regardless of how the process exits.
    onExit(
      () => {
        this.writeCoverageFile();
